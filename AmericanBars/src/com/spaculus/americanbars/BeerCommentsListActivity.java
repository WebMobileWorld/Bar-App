...
public class BeerCommentsListActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		try {
			...
			//  Create Action Bar
			createActionBar(ConfigConstants.Titles.TITLE_BEER_COMMENTS, R.layout.custom_actionbar, BeerCommentsListActivity.this, true);
			...
			adapter = new CustomCommentsAdapter(BeerCommentsListActivity.this, R.layout.activity_comments_list_item, beerCommentsList, BeerCommentsListActivity.this, ConfigConstants.Titles.TITLE_BEER_COMMENTS);
			listViewBeerComments.setAdapter(adapter);
			
			//  Now, get the Comments list here
			if(Utils.getInstance().isNetworkAvailable(BeerCommentsListActivity.this)) {
				if (objAsynTaskGetBeerCommentsList == null) {
					if (Utils.getInstance().isNetworkAvailable(BeerCommentsListActivity.this)) {
						objAsynTaskGetBeerCommentsList = new AsynTaskGetBeerCommentsList();
						objAsynTaskGetBeerCommentsList.execute();
					} else {
						Utils.getInstance().showToastNoInternetAvailable(BeerCommentsListActivity.this);
					}
				}
			}
			else{
				...
				public void onClick(View v) {
					try {
						...
						if(validationResponse.isEmpty()) {
							...
							Utils.toastLong(BeerCommentsListActivity.this, validationResponse);
						}
					}
					catch (Exception e) {
						e.printStackTrace();
	...
	private void mappedAllViews() {
		try {
			...
			listViewBeerComments = (LoadMoreListView)findViewById(R.id.listViewComments);
	...
	/* An AnyncTask to get the Beer Comments List. */
	public class AsynTaskGetBeerCommentsList extends AsyncTask<String, Void, String> {
		private ProgressDialog pd = new ProgressDialog(BeerCommentsListActivity.this);
    	@Override
		...
		protected String doInBackground(String... params) {
			try {
	        	...
	        	nameValuePairs.add(new BasicNameValuePair("user_id", SessionManager.getInstance(BeerCommentsListActivity.this).getData(SessionManager.KEY_USER_ID)));
		        nameValuePairs.add(new BasicNameValuePair("device_id", SessionManager.getInstance(BeerCommentsListActivity.this).getData(SessionManager.KEY_DEVICE_ID)));
		        nameValuePairs.add(new BasicNameValuePair("unique_code", SessionManager.getInstance(BeerCommentsListActivity.this).getData(SessionManager.KEY_UNIQUE_CODE)));
				...
				String responseString = sh.makeServiceCall(ConfigConstants.Urls.beer_comments, ServiceHandler.POST, nameValuePairs);
		...
		protected void onPostExecute(String result) {
			try {
				...
				listViewBeerComments.onLoadMoreComplete();
				try {
					...
					if(result != null){
						...
						if(status.equals(ConfigConstants.Messages.RESPONSE_SUCCESS)){
							totalRecords = jsonObjParent.getInt("beer_comment_total");
							...
							if(totalRecords > 0) {
								...
								if(jsonObjParent.has("beer_comment")) {
									JSONArray jsonArrayBarComment = jsonObjParent.getJSONArray("beer_comment");
									if(jsonArrayBarComment != null) {
										if(jsonArrayBarComment.length() > 0) {
											...
											listViewBeerComments.setVisibility(View.VISIBLE);
											...
											if(!isLoadMoreButtonClicked) {
												listViewBeerComments.setSelection(0);
											}
											// Call onLoadMoreComplete when the LoadMore task, has finished
											((LoadMoreListView) listViewBeerComments).onLoadMoreComplete();
										}
										else {
											//  A method to called when no data is available
											noDataAvailable();
			...
			objAsynTaskGetBeerCommentsList = null;
	...
	private void noDataAvailable() {
		try {
			if (isLoadMoreButtonClicked) {
				...
				listViewBeerComments.hideFooterText();
				Utils.showToastLong(BeerCommentsListActivity.this, ConfigConstants.Messages.noMoreRecordFound);
			} 
			else {
				listViewBeerComments.hideFooterText();
				listViewBeerComments.setVisibility(View.GONE);
				//tvNoReviews.setVisibility(View.VISIBLE);
				Utils.showToastLong(BeerCommentsListActivity.this, ConfigConstants.Messages.noRecordFound);
			}
		} 
		catch (Exception e) {
			e.printStackTrace();
		...
		protected void onPostExecute(String response) {
			try {
				...
				if (response != null) {
					...
					String status = Utils.getInstance().isTagExists(jsonObjParent, "status");
					if(status.equals(ConfigConstants.Messages.RESPONSE_SUCCESS)) {
						Utils.getInstance().displayToast(BeerCommentsListActivity.this, "Your comment is added successfully.");
						ConfigConstants.isBeerCommentAdded = true;
						...
						Utils.getInstance().displayToast(BeerCommentsListActivity.this, "Your comment is not added. Please try again.");
					}
				} 
			} catch (Exception e) {
				e.printStackTrace();
	...
	public void likeBeerComment(int position) {
		if(Utils.getInstance() .isNetworkAvailable(BeerCommentsListActivity.this)) {
			if (objAsynTaskLikeBeerComment == null) {
				if (Utils.getInstance().isNetworkAvailable(BeerCommentsListActivity.this)) {
					objAsynTaskLikeBeerComment = new AsynTaskLikeBeerComment();
					objAsynTaskLikeBeerComment.execute(String.valueOf(position));
				} else {
					Utils.getInstance().showToastNoInternetAvailable(BeerCommentsListActivity.this);
				}
			}
			...
			try {
				...
				if (response != null) {
					...
					if(status.equals(ConfigConstants.Messages.RESPONSE_SUCCESS)) {
						...
						Utils.getInstance().displayToast(BeerCommentsListActivity.this, "Please try again.");
					}
				} 
			} catch (Exception e) {
				e.printStackTrace();
