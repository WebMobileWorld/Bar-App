...
public class CocktailCommentRepliesActivity extends BaseActivity {
	...
	private String cocktail_id = "", master_comment_id = "";
	...
	protected void onCreate(Bundle savedInstanceState) {
		try {
			...
			//  Create Action Bar
			createActionBar(ConfigConstants.Titles.TITLE_COCKTAIL_COMMENT_REPLIES, R.layout.custom_actionbar, CocktailCommentRepliesActivity.this, true);
			...
			adapter = new CustomCommentRepliesAdapter(CocktailCommentRepliesActivity.this, R.layout.activity_replies_list_item, replyList, CocktailCommentRepliesActivity.this, ConfigConstants.Titles.TITLE_COCKTAIL_COMMENT_REPLIES);
			listViewReplies.setAdapter(adapter);
			
			//  Now, get the Comments list here
			if(Utils.getInstance().isNetworkAvailable(CocktailCommentRepliesActivity.this)) {
				if (objAsynTaskGetReplyList == null) {
					if (Utils.getInstance().isNetworkAvailable(CocktailCommentRepliesActivity.this)) {
						objAsynTaskGetReplyList = new AsynTaskGetReplyList();
						objAsynTaskGetReplyList.execute();
					} else {
						Utils.getInstance().showToastNoInternetAvailable(CocktailCommentRepliesActivity.this);
					}
				}
			}
			else{
				...
				public void onClick(View v) {
					try {
						...
						if(validationResponse.isEmpty()) {
							...
							Utils.toastLong(CocktailCommentRepliesActivity.this, validationResponse);
						}
					}
					catch (Exception e) {
						e.printStackTrace();
	...
	public class AsynTaskGetReplyList extends AsyncTask<String, Void, String> {
		private ProgressDialog pd = new ProgressDialog(CocktailCommentRepliesActivity.this);
    	@Override
		...
		protected String doInBackground(String... params) {
			try {
	        	...
	        	nameValuePairs.add(new BasicNameValuePair("user_id", SessionManager.getInstance(CocktailCommentRepliesActivity.this).getData(SessionManager.KEY_USER_ID)));
		        nameValuePairs.add(new BasicNameValuePair("device_id", SessionManager.getInstance(CocktailCommentRepliesActivity.this).getData(SessionManager.KEY_DEVICE_ID)));
		        nameValuePairs.add(new BasicNameValuePair("unique_code", SessionManager.getInstance(CocktailCommentRepliesActivity.this).getData(SessionManager.KEY_UNIQUE_CODE)));
				...
				String responseString = sh.makeServiceCall(ConfigConstants.Urls.cocktail_subcomments, ServiceHandler.POST, nameValuePairs);
				...
				try {
					...
					if(result != null){
						...
						if(status.equals(ConfigConstants.Messages.RESPONSE_SUCCESS)){
							totalRecords = jsonObjParent.getInt("cocktail_subcomment_total");
							...
							if(totalRecords > 0) {
								...
								if(jsonObjParent.has("cocktail_subcomment")) {
									JSONArray jsonArrayBarComment = jsonObjParent.getJSONArray("cocktail_subcomment");
									if(jsonArrayBarComment != null) {
										if(jsonArrayBarComment.length() > 0) {
											for (int i = 0; i < jsonArrayBarComment.length(); i++) {
												...
												replyList.add(new ParseJsonObject().addCocktailReplyObject(jsonObj));
											}
	...
	private void noDataAvailable() {
		try {
			if (isLoadMoreButtonClicked) {
				...
				Utils.showToastLong(CocktailCommentRepliesActivity.this, ConfigConstants.Messages.noMoreRecordFound);
			} 
			else {
				listViewReplies.hideFooterText();
				listViewReplies.setVisibility(View.GONE);
				//tvNoReviews.setVisibility(View.VISIBLE);
				Utils.showToastLong(CocktailCommentRepliesActivity.this, ConfigConstants.Messages.noRecordFound);
			}
		} 
		catch (Exception e) {
			e.printStackTrace();
	...
	public class AsynTaskPostReply extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			try {
	        	...
	        	// Making a request to URL and getting response
		        String responseString = sh.makeServiceCall(ConfigConstants.Urls.add_cocktail_subcomment, ServiceHandler.POST, nameValuePairs);
		...
		protected void onPostExecute(String response) {
			try {
				...
				if (response != null) {
					...
					String status = Utils.getInstance().isTagExists(jsonObjParent, "status");
					if(status.equals(ConfigConstants.Messages.RESPONSE_SUCCESS)) {
						Utils.getInstance().displayToast(CocktailCommentRepliesActivity.this, "Your reply is added successfully.");
						...
						Utils.getInstance().displayToast(CocktailCommentRepliesActivity.this, "Your reply is not added. Please try again.");
					}
				} 
			} catch (Exception e) {
				e.printStackTrace();
	...
	public void showAlertDeleteReply(int position) {
		try {
			FragmentAlertDialogDelete objDeleteReply = new FragmentAlertDialogDelete("Delete Reply", "Are you sure you want to delete this reply?", "Yes", "No", 
					ConfigConstants.Titles.TITLE_COCKTAIL_COMMENT_REPLIES, CocktailCommentRepliesActivity.this, position);
	...
	public class AsynTaskDeleteReply extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			try {
		        ...
		        nameValuePairs.add(new BasicNameValuePair("device_id", SessionManager.getInstance(CocktailCommentRepliesActivity.this).getData(SessionManager.KEY_DEVICE_ID)));
		        nameValuePairs.add(new BasicNameValuePair("unique_code", SessionManager.getInstance(CocktailCommentRepliesActivity.this).getData(SessionManager.KEY_UNIQUE_CODE)));
		        nameValuePairs.add(new BasicNameValuePair("cocktail_comment_id", replyList.get(position).getComment_id()));
	        	// Making a request to URL and getting response
		        String responseString = sh.makeServiceCall(ConfigConstants.Urls.remove_cocktail_comment, ServiceHandler.POST, nameValuePairs);
	        	Log.i("responseString",responseString);
				return responseString;
			} catch (Exception e) {
				e.printStackTrace();
			...
			try {
				...
				if (response != null) {
					...
					if(status.equals(ConfigConstants.Messages.RESPONSE_SUCCESS)) {
						...
						Utils.getInstance().displayToast(CocktailCommentRepliesActivity.this, "Please try again.");
					}
				} 
			} catch (Exception e) {
				e.printStackTrace();
